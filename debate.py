...
from prompts import (
    ...
    SYSTEM_R1, SYSTEM_JUDGE, SYSTEM_SYNTHESIZER, SYSTEM_TRANSLATOR,
)
...
_GROK_MODEL = os.environ.get("GROK_MODEL", "grok-4.3")
...
def _save_roles(session_id, roles: dict):
    roles_text = "\n".join(f"{alias}: {desc}" for alias, desc in roles.items())
    _insert(session_id, 0, "[Roles]", "system", "roles", roles_text)
...
def run_debate(topic: str, framework: str, max_rounds: int = 4) -> str:
    ...
    # Export TXT — English version (utf-8-sig so Windows reads accents correctly)
    en_path = Path(__file__).parent / f"debate_{timestamp}_EN.txt"
    header = (
    ...
    en_content = header + _format_export(session_id)
    en_path.write_text(en_content, encoding="utf-8-sig")
    print(f"\nSaved (EN): {en_path}")

    # Spanish translation — generated by Grok with high token limit for full content
    print("\nGenerating Spanish translation (ES)...")
    es_raw = _grok_client.chat.completions.create(
        model=_GROK_MODEL, temperature=0.2, max_tokens=32000,
        messages=[{"role": "system", "content": SYSTEM_TRANSLATOR},
                  {"role": "user",   "content": en_content}],
    )
    es_content = es_raw.choices[0].message.content.strip()
    es_path = Path(__file__).parent / f"debate_{timestamp}_ES.txt"
    es_path.write_text(es_content, encoding="utf-8-sig")
    print(f"Saved (ES): {es_path}")
